    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Login(string taiKhoan, string matKhau)
        {
            ...
            if (userRole != null)
            {
                ...
                if (userRole == "ADMIN")
                {
                    return RedirectToAction("Authentication", "HomeAD");
                }
                else
                {
            ...
            if (Session["User"] != null)
            {
                ...
                using (var connection = new OracleConnection(db.GetConnectionString()))
                {
                    using (var command = new OracleCommand("KiemtraSession", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add("p_username", OracleDbType.Varchar2).Value = username;
                        command.Parameters.Add("p_sessionID", OracleDbType.Varchar2, 50).Direction = ParameterDirection.Output;

                        connection.Open();
                        command.ExecuteNonQuery();

                        // Lấy giá trị của p_sessionID nếu cần
                        string sessionID = command.Parameters["p_sessionID"].Value.ToString();
                    }
                }
            }
