...
ALTER SESSION SET CONTAINER = VUNGNUOI1;
SELECT logins FROM v$instance;
...
--TAO TABLESPACE
CREATE TABLESPACE VUNGNUOI_TABLESPACE2
  DATAFILE 'D:\CUNGNHAURATRUONG\VUNGNUOI_TABLESPACE2.dbf' 
  SIZE 100M 
  AUTOEXTEND ON 
  NEXT 10M 
  MAXSIZE UNLIMITED;
--KIEM TRA 
SELECT TABLESPACE_NAME, STATUS FROM DBA_TABLESPACES WHERE TABLESPACE_NAME = 'VUNGNUOI_TABLESPACE2';


SELECT TABLESPACE_NAME FROM DBA_TABLESPACES WHERE
-----------------------------------------------------------------------------------------------------alter user vungnuoi30 identified by vungnuoi3;
--b1
CREATE PLUGGABLE DATABASE VUNGNUOI2
  ADMIN USER VUNGNUOI1 IDENTIFIED BY vungnuoi1
  FILE_NAME_CONVERT = ('C:\u01\app\oracle\oradata\ORCL21\pdbseed', 'D:\CUNGNHAURATRUONG\ORCL2');
--b2
ALTER PLUGGABLE DATABASE VUNGNUOI1 OPEN;
--b3 cap quyen
ALTER SESSION SET CONTAINER = VUNGNUOI1;

ALTER USER VUNGNUOI1 QUOTA UNLIMITED ON VUNGNUOI_TABLESPACE2;

GRANT ALL PRIVILEGES TO VUNGNUOI1;

GRANT CREATE SESSION TO VUNGNUOI1;
GRANT CREATE USER TO VUNGNUOI1;
GRANT DBA TO VUNGNUOI1; 
GRANT UNLIMITED TABLESPACE TO VUNGNUOI1;
ALTER USER VUNGNUOI1 QUOTA UNLIMITED ON VUNGNUOI_TABLESPACE2; 
GRANT EXECUTE ON DBMS_CRYPTO TO VUNGNUOI1;

----------------------------------------------------------------------------------------------------nho thay password 
CREATE PROFILE vungnuoiAD LIMIT PASSWORD_LIFE_TIME UNLIMITED;
ALTER USER VUNGNUOI1 PROFILE vungnuoiAD;
...
------------------------------------------------------------------------------------------------tao sequence phuc vu cho viec code 
...
CREATE PROFILE Nguoi_Dung1 LIMIT
    ...
    PASSWORD_LOCK_TIME 1/1440; -- 1 ph?t (1/1440 c?a m?t ng?y)
    
    
---------------------------------------------------------------------------------------Profile Admin
alter user "admin" profile Quan_TriVien
...
BEGIN
    ...
    -- Thay ??i profile t? DEFAULT th?nh Nguoi_Dung1
...
---------------------------------------------------------------------------------------cap nhat session cho nguoi dung dang nhap
...
---------------------------------------------------------------------------------------kiem tra xem session nay co hop le khong
